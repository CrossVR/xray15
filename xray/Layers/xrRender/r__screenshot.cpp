		...
		case IRender_interface::SM_FOR_GAMESAVE:
			{
				...
				CHK_DX(D3DX11LoadTextureFromTexture( HW.pDevice, pSrcTexture,
					NULL, pSrcSmallTexture ));

				// save (logical & physical)
				ID3DBlob*		saved	= 0;
				HRESULT hr					= D3DX11SaveTextureToMemory( HW.pDevice, pSrcSmallTexture, D3DX11_IFF_DDS, &saved, 0);
		...
		case IRender_interface::SM_NORMAL:
			{
				...
				ID3DBlob			*saved	= 0;
				CHK_DX				(D3DX11SaveTextureToMemory( HW.pDevice, pSrcTexture, D3DX11_IFF_JPG, &saved, 0));
				...
				{ // hq
					...
					ID3DBlob*		saved	= 0;
					CHK_DX				(D3DX11SaveTextureToMemory( HW.pDevice, pSrcTexture, D3DX11_IFF_BMP, &saved, 0));
...
void CRender::ScreenshotAsyncEnd(CMemoryWriter &memory_writer)
{
	...
	HW.pDevice->Map(pTex, 0, D3D11_MAP_READ, 0, &MappedData);
