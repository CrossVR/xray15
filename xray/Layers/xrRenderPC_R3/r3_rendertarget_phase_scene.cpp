...
void	CRenderTarget::phase_scene_prepare	()
{
		...
		)
	{
      ...
      if( !RImplementation.o.dx10_msaa )
         HW.pDevice->ClearDepthStencilView(HW.pBaseZB, D3D11_CLEAR_DEPTH|D3D11_CLEAR_STENCIL, 1.0f, 0);
      else
      {
         HW.pDevice->ClearRenderTargetView(rt_Color->pRT, ColorRGBA);
				 HW.pDevice->ClearRenderTargetView(rt_Accumulator->pRT, ColorRGBA);
				 HW.pDevice->ClearDepthStencilView(rt_MSAADepth->pZRT, D3D11_CLEAR_DEPTH|D3D11_CLEAR_STENCIL, 1.0f, 0);
         HW.pDevice->ClearDepthStencilView(HW.pBaseZB, D3D11_CLEAR_DEPTH|D3D11_CLEAR_STENCIL, 1.0f, 0);
      }
   }
	else
